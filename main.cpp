...
void BoatDispatch()
{
    ...
    { // 对于每艘船
        ...
        { // 装载状态进行讨论
            ...
            Boats[bi].pre_dest_berth = Boats[bi].dest_berth;
        ...
        { // 行驶状态，寻路
            ...
            if (Boats[bi].dest_berth != -1)
            {
                ...
                { //到港口
                    ...
                    Boats[bi].pre_dest_berth = Boats[bi].dest_berth; //初始到港口之后
                }
            }
    ...
    if (Frame == 1)
    {
        for (int i = 0; i < MAX_BOAT_NUM; i++)
        {
            if (InitBuyingToBuy[i] == -1)
                ...
                Boats[BoatNum-1].dest_berth = InitBerthToGo[BoatNum-1]; //确定初始船要去的港口
        }
    }
    else  // 其余时刻船的购买策略
    {
        ...
        else
        {
            for (int bi = 0; bi < BerthNum; bi++)
            {
                if (Berths[bi].boat_id == -1 && Berths[bi].goods_queue.size() > BERTH_GOODS_TOO_MUCH)
                {
